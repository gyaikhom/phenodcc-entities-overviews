...
@NamedQueries({
    ...
    @NamedQuery(name = "ProcedureAnimalOverview.findByCidGidSidQeid", query = "SELECT DISTINCT new org.mousephenotype.dcc.entities.overviews.ProcedureMetadataGroup(p.pipeline, p.procedureId, p.metadataGroup) FROM ProcedureAnimalOverview p, MeasurementsPerformed m WHERE (p.procedureOccurrenceId = m.procedureOccurrenceId AND p.centreId = :centreId AND p.genotypeId = :genotypeId AND p.strainId = :strainId AND m.parameterId = :parameterId)"),
    @NamedQuery(name = "ProcedureAnimalOverview.findByCidLidGidSidPeidQeid", query = "SELECT DISTINCT new org.mousephenotype.dcc.entities.overviews.ProcedureMetadataGroup(p.pipeline, p.procedureId, p.metadataGroup) FROM ProcedureAnimalOverview p, MeasurementsPerformed m, Pipeline l WHERE (p.procedureOccurrenceId = m.procedureOccurrenceId AND p.centreId = :centreId AND l.pipelineId = :pipelineId AND p.pipeline = l.pipelineKey AND p.genotypeId = :genotypeId AND p.strainId = :strainId AND p.procedureId = :procedureId AND m.parameterId = :parameterId)")})
